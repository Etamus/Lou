...
class AIFeaturesMixin:
    ...
    def _sanitize_and_split_response(self, text: str) -> list:
        """
        O 'Editor Gramatical Definitivo'. Limpa e divide a resposta da IA de forma natural,
        usando uma combinação de quebras de linha, capitalização e regras gramaticais.
        """
        ...
        # Etapa 1: Limpeza Inicial Global de pontuação e emojis indesejados
        ...
        text = text.replace('!', '')
        # Remove pontos finais, mas preserva reticências
        if text.endswith('.') and not text.endswith('...'):
            text = text[:-1]

        # Etapa 2: Aplica os separadores. A '|' é um caractere que raramente será usado.
        # Regra 2a: Quebra de linha é um separador inquestionável.
        text_with_separators = text.replace('\n', '|')
        # Regra 2b: Um espaço seguido por uma letra maiúscula (no meio da frase) também é um separador.
        text_with_separators = re.sub(r'(?<=[a-z,?!…])\s+([A-Z])', r'|\1', text_with_separators)

        # Agora temos uma lista de "candidatos" a chunks
        chunks = [chunk.strip() for chunk in text_with_separators.split('|') if chunk.strip()]

        # Etapa 3: Limpeza e Capitalização Final
        final_chunks = []
        for i, chunk in enumerate(chunks):
            # Remove vírgulas ou pontos que podem ter ficado no final de um chunk
            clean_chunk = chunk.strip().rstrip('.,')
            
            if not clean_chunk:
                continue

            # Garante que novas frases (a partir da segunda) comecem com letra maiúscula
            if i > 0 and clean_chunk and clean_chunk[0].islower():
