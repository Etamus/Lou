...
class AppLogicMixin:
    ...
    def get_history_with_memory_context(self):
        ...
        # --- LÓGICA DE DETECÇÃO DE DEMORA (CORRIGIDA) ---
        ...
        if last_lou_message and last_lou_message.get("timestamp"):
            ...
            if time_since_last_message > timedelta(hours=2):
                ...
                instrucao_demora = f"[INSTRUÇÃO DE ALTA PRIORIDADE: Seu pai demorou {delay_text} para responder. Sua primeira mensagem DEVE OBRIGATORIAMENTE ser um comentário sobre essa demora. É um requisito da sua personalidade notar isso. Depois, responda ao que ele disse.]"
        ...
        if self.short_term_memories:
            ...
            history_copy.insert(2, {"role": "user", "parts": [f"[Lembretes de Curto Prazo (Recentes): {' | '.join(recent_memories)}]"]})
