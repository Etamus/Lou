...
class ContextUpdateWorker(BaseWorker):
    ...
    def run(self):
        try:
            ...
            prompt = f"""
            Sua tarefa é analisar a conversa e extrair informações que são NOVAS e RELEVANTES.
            INFORMAÇÕES JÁ SALVAS (NÃO REPITA NEM REFORMULE):
            - Memórias de Curto Prazo Atuais: {memories_str}
            - Estilos Atuais: {styles_str}
            CONVERSA RECENTE PARA ANÁLISE:
            {self.snippet}
            INSTRUÇÕES:
            1. Analise a 'CONVERSA RECENTE'.
            2. Extraia fatos de CURTO PRAZO (memories) e padrões de escrita do usuário (styles) que sejam GENUINAMENTE NOVOS.
            3. NÃO extraia informações que já existem nas listas 'INFORMAÇÕES JÁ SALVAS' ou que são apenas uma pequena variação delas.
            ...
            if start_index != -1 and end_index != -1 and start_index < end_index:
                ...
                context_data = {"memories": data.get("memories", []),"styles": data.get("styles", [])}
...
class ProactiveMessageWorker(BaseWorker):
    ...
    def run(self):
        ...
        if self.attempt_count < 2:
            prompt = """
            Sua tarefa é gerar uma mensagem proativa curta e natural para reengajar um usuário que está quieto.

            DIRETRIZES TÉCNICAS (OBRIGATÓRIAS):
            1.  Sua mensagem DEVE ser um pensamento completo e autônomo.
            ...
            Exemplos: "Pai?", "Ta aí?", "Tudo bem por aí?"
            """
        try:
