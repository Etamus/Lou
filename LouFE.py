...
class BaseDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet("""
            ...
            QPushButton { background-color: #4f545c; color: #fff; border: none; padding: 8px 16px; border-radius: 3px; font-weight: bold;} 
...
class RenameDialog(BaseDialog):
    def __init__(self, current_name, parent=None):
        ...
        self._add_close_button_to_layout(self.layout) # Adiciona o botão 'X'
...
class ConfirmationDialog(BaseDialog):
    def __init__(self, title, message, parent=None, add_close_button=True): # Novo parâmetro aqui
        super().__init__(parent)
        self.layout = QVBoxLayout(self)
        
        # Adiciona o botão 'X' apenas se a condição for verdadeira
        if add_close_button:
...
class ServerSettingsDialog(BaseDialog):
    def __init__(self, current_name, current_avatar_path, parent=None):
        ...
        self.input = QLineEdit(current_name); self.input.setObjectName("dialog_input")
        self.buttons = QHBoxLayout()
        self.delete_button = QPushButton("Excluir"); self.delete_button.setStyleSheet("background-color: #d83c3e;")
        self.save_button = QPushButton("Salvar"); self.save_button.clicked.connect(self.accept)
        self.buttons.addWidget(self.delete_button); self.buttons.addStretch(); self.buttons.addWidget(self.save_button)
        self.layout.addWidget(self.avatar_label, 0, Qt.AlignCenter); self.layout.addWidget(change_avatar_button)
        self.layout.addWidget(self.label); self.layout.addWidget(self.input); self.layout.addLayout(self.buttons)
...
class CreateServerDialog(BaseDialog):
    def __init__(self, parent=None):
        ...
        self.avatar_button = QPushButton("Enviar Ícone"); self.avatar_button.clicked.connect(self.change_avatar)
        self.name_label = QLabel("NOME DO SERVIDOR"); self.name_label.setObjectName("dialog_label")
        self.name_input = QLineEdit(); self.name_input.setObjectName("dialog_input"); self.name_input.setPlaceholderText("Ex: Clube de Games")
        self.buttons = QHBoxLayout(); self.ok_button = QPushButton("Criar"); self.ok_button.clicked.connect(self.accept)
        self.buttons.addStretch(); self.buttons.addWidget(self.ok_button)
        self.layout.addWidget(self.avatar_label, 0, Qt.AlignCenter); self.layout.addWidget(self.avatar_button)
        self.layout.addWidget(self.name_label); self.layout.addWidget(self.name_input); self.layout.addLayout(self.buttons)
    def change_avatar(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Selecionar Ícone do Servidor", "", "Imagens (*.png *.jpg *.jpeg)")
        if file_path: self.new_avatar_path = file_path; self.avatar_label.set_avatar(file_path)
...
class UserSettingsDialog(BaseDialog):
    def __init__(self, current_name, current_avatar_path, parent=None):
        ...
        change_avatar_button = QPushButton("Alterar avatar"); change_avatar_button.clicked.connect(self.change_avatar)
        ...
        self.buttons = QHBoxLayout(); self.save_button = QPushButton("Salvar"); self.save_button.clicked.connect(self.accept)
        self.buttons.addStretch(); self.buttons.addWidget(self.save_button)
        self.layout.addWidget(self.avatar_label, 0, Qt.AlignCenter); self.layout.addWidget(change_avatar_button)
        self.layout.addWidget(self.name_label); self.layout.addWidget(self.name_input); self.layout.addLayout(self.buttons)
    def change_avatar(self):
